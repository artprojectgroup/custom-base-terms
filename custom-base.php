...
define( 'VERSION_custom_base_terms', '1.1.0' );
...
//Carga el idioma. En init, porque cargarlo antes dispara el aviso de traducción prematura de WordPress 6.7.
function custom_base_terms_idioma() {
	load_plugin_textdomain( 'custom-base-terms', false, dirname( DIRECCION_custom_base_terms ) . '/languages' );
}
add_action( 'init', 'custom_base_terms_idioma' );
...
function custom_base_terms_inicio() {
	...
	if ( empty( $bases ) || ! $wp_rewrite instanceof WP_Rewrite ) {
		return;
	}
...
function custom_base_terms_seccion() {
	global $bases, $wp_rewrite, $custom_base_terms;
...
function custom_base_terms_estilo( $hook ) {
	...
	wp_enqueue_style( 'custom_base_terms_hoja_de_estilo', plugins_url( 'assets/css/style.css', __FILE__ ), array(), VERSION_custom_base_terms ); //Carga la hoja de estilo
}
...
function custom_base_terms_desinstalar() {
	...
	$bases = array( 'author_base', 'search_base', 'comments_base', 'pagination_base', 'feed_base' );
